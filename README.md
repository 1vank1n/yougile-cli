...
pager: ""              # зарезервировано: пейджер пока не используется
aliases:
...
| `YOUGILE_PAGER` | зарезервировано вместе с настройкой `pager`; пейджер пока не используется |
...
4. `--download` качает всё найденное потоком, с заголовком `Authorization: Bearer`,
   в каталог из `--dir`. Существующие файлы не перезаписываются без `--force`.
